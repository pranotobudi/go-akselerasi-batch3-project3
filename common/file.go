...
func UploadFile(authorID uint, file multipart.File, filename string) (string, error) {
	...
	resp, err := cld.Upload.Upload(ctx, file, uploader.UploadParams{PublicID: "authorID-" + fmt.Sprintf("%d", authorID) + "-" + filename})
	...
	asset, err := cld.Admin.Asset(ctx, admin.AssetParams{PublicID: "authorID-" + fmt.Sprintf("%d", authorID) + "-" + filename})
	if err != nil {
